...
`image_url` must point to a reachable `http://` or `https://` JPEG. The JPEG download is limited to 3.5 MiB and source image area up to 1920x1920 pixels. For Home Assistant cameras, save the snapshot in `/config/www`; Home Assistant serves that folder under `/local`.
