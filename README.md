...
AirDot combines real-time air-quality sensing, a sharp 480 x 480 color display, local setup, standalone operation, optional Home Assistant integration, optional MQTT integration, time, weather, Flight Radar, alerts, and on-device history pages in one compact ESPHome-powered device.
...
MQTT is available for users who prefer an MQTT-based setup. Broker, credentials, topic prefix, discovery, control entities, and publishing interval can be configured from the setup page.
...
To configure AirDot for the first time:
...
You can reopen setup at any time with the physical action button.

1. Wake the display if needed.
2. Long-press the action button for about 0.8 seconds.
3. Follow the instructions shown on the AirDot display.

If AirDot is already connected to Wi-Fi, setup opens on your local network. The display shows the local browser address, for example `http://192.168.1.42`. Open that address from a phone, tablet, or computer on the same network.

If AirDot is not connected to Wi-Fi, it starts its setup access point instead. Connect to the AirDot Wi-Fi network shown on the display, then open `http://192.168.4.1`.

Long-pressing again while the setup screen is open closes setup and returns to the normal display pages.
...
`show_snapshot_alert` accepts:

- `image_url`
- `duration_seconds`
- `sound_enabled`

`image_url` must point to a reachable `http://` or `https://` JPEG/JPG image. The JPEG download is limited to 8 MiB and source image area up to 1920x1920 pixels. Local Home Assistant URLs such as `http://homeassistant.local:8123/local/front-door.jpg` are supported when the device can reach that address. For Home Assistant cameras, save the snapshot in `/config/www`; Home Assistant serves that folder under `/local`.
...
- action: esphome.airdot_show_snapshot_alert
  data:
    image_url: http://homeassistant.local:8123/local/front-door.jpg
    duration_seconds: 30
    sound_enabled: true
```
...
MQTT discovery exposes the same main sensor and control entity names as the ESPHome API where supported. MQTT also exposes the Action Button state when MQTT discovery is enabled.
...
- Automatic page switching: all pages or a custom selected set
...
3. Open the local address shown on the AirDot display.
4. If AirDot is not connected to Wi-Fi, connect to the AirDot setup network first and open `http://192.168.4.1`.
